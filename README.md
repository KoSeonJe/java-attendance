...
3. 예외가 발생할 일이 없다면, 예외 처리를 해야할까? 예외 처리를 한다면 테스트를 어떻게 할 수 있을까? (domain.SchoolDay)
...
6. 객체를 생성 후 생성 결과를 출력할 때, 
   출석 처리 로직에서 에러가 나지 않았으니 단순히 생성하는 데 사용되었던 변수를 사용해서 출력하는 것이 좋을까?
   아니면, 생성된 객체를 다시 조회해서 해당 객체를 사용해 결과를 출력하는 것이 좋을까?
