...
3. AttendanceStatus에서 findByStartHourAndAttendanceTime() 내부 구현 중 모든 조건에 부합하지 않는 값이 없는데,
    예외 발생 테스트를 어떻게 작성해야 하는가? 출석 조건을 체크하지 말아야 하는 것일까?
    하지만, 제 의도는 출석도 명확한 조건을 만들고 싶었다.. 다른 추가 조건이 생길지도 모르니까?
...
### 기능
- [x] 출석 시간을 수정한다
    - [ ] 주말 혹은 공휴일이 아니며 출석 기록이 없는 날짜는 결석으로 간주한다
- [ ] 출석 상태를 수정한다
